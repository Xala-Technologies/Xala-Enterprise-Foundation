...
# {{PACKAGE_NAME}} Package - Install Development Hooks
# Sets up local development environment with validation hooks

echo "🚀 Installing {{PACKAGE_NAME}} development hooks..."
...
# Check if we're in the {{PACKAGE_NAME}} package
if [[ ! -f "package.json" ]]; then
    print_error "This script must be run from the {{PACKAGE_NAME}} package root"
    exit 1
fi
